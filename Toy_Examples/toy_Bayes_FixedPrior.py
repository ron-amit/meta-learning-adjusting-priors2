...
def learn(data_set):
    ...
    w_P_mu = torch.zeros(n_dim, requires_grad=False, device=device)
    w_P_log_sigma = torch.zeros(n_dim, requires_grad=False, device=device)
    ...
    for i_epoch in range(n_epochs):
        ...
        epsilon = torch.randn(n_dim, requires_grad=False).to(device)
